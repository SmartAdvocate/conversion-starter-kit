...
*/
-- select distinct addr_Type from  [BenAbbot_Needles].[dbo].[multi_addresses]
-- select * from  [BenAbbot_Needles].[dbo].[multi_addresses] where addr_type not in ('Home','business', 'other')
...
insert into [sma_MST_Address]
	...
	from [BenAbbot_Needles].[dbo].[multi_addresses] a
	join [sma_MST_Indvcontacts] i
		on i.source_id = CONVERT(VARCHAR(MAX), a.namesid)
...
insert into [sma_MST_Address]
	...
	join [sma_MST_AddressTypes] t
		on t.addnContactCategoryID = i.cinnContactCtg
			and t.addsCode = 'OTH'
	where
		A.[addr_type] not in ('Business', 'Home')
		and
		(   ISNULL(a.[address], '') <> ''
			or
			ISNULL(a.[address_2], '') <> ''
...
-- 	 from [BenAbbot_SA].[dbo].[sma_MST_Indvcontacts] I 
--	 inner join [BenAbbot_SA].[dbo].[sma_MST_Address] A on A.addnContactID=I.cinnContactID and A.addnContactCtgID=I.cinnContactCtg and A.addbPrimary=1 
--	 group by cinnContactID
--	 having count(cinnContactID)>1

-- select O.connContactID
-- 	 from [BenAbbot_SA].[dbo].[sma_MST_OrgContacts] O 
--	 inner join [BenAbbot_SA].[dbo].[sma_MST_Address] A on A.addnContactID=O.connContactID and A.addnContactCtgID=O.connContactCtg and A.addbPrimary=1 
