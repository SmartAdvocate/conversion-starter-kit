...
/*
alter table [BenAbbot_Needles].[dbo].[sma_TRN_Lienors] disable trigger all
delete from [BenAbbot_Needles].[dbo].[sma_TRN_Lienors] 
DBCC CHECKIDENT ('[BenAbbot_Needles].[dbo].[sma_TRN_Lienors]', RESEED, 0);
alter table [BenAbbot_Needles].[dbo].[sma_TRN_Lienors] enable trigger all

alter table [BenAbbot_Needles].[dbo].[sma_TRN_LienDetails] disable trigger all
delete from [BenAbbot_Needles].[dbo].[sma_TRN_LienDetails] 
DBCC CHECKIDENT ('[BenAbbot_Needles].[dbo].[sma_TRN_LienDetails]', RESEED, 0);
alter table [BenAbbot_Needles].[dbo].[sma_TRN_LienDetails] enable trigger all
...
select count(*) from [BenAbbot_Needles].[dbo].[sma_TRN_Lienors]

select * from value_tab_Liencheckbox_Helper 

select * from [BenAbbot_Needles].[dbo].[value_payment] where value_id=65990
...
INSERT INTO value_tab_Liencheckbox_Helper
	...
	SELECT
		VP1.value_id
	FROM [BenAbbot_Needles].[dbo].[value_payment] VP1
	LEFT JOIN (
		SELECT DISTINCT
			value_id
		FROM [BenAbbot_Needles].[dbo].[value_payment]
		WHERE lien = 'Y'
	) VP2
...
INSERT INTO [SA].[dbo].[sma_MST_LienType]
   ...
   ,[lntsDscrptn]
	...
	SELECT DISTINCT
		'CONVERSION'
	   ,VC.[description]
	FROM [BenAbbot_Needles].[dbo].[value] V
	INNER JOIN [BenAbbot_Needles].[dbo].[value_code] VC
		ON VC.code = V.code
	...
	SELECT
	   ...
	   ,(
			...
			WHERE lntsDscrptn = (
					SELECT TOP 1
						[description]
					FROM [BenAbbot_Needles].[dbo].[value_code]
					WHERE [code] = V.code
				)
		)						 
		AS [lnrnLienorTypeID]
