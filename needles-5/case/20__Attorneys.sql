...
insert into [sma_TRN_PlaintiffAttorney]
	...
	select distinct
		...
		''				  as [plascomments]
	--select *
	from [BenAbbot_Needles]..[counsel_Indexed] c
	--left join [BenAbbot_Needles].[dbo].[user_counsel_data] ud
	--	on ud.counsel_id = c.counsel_id
	--		and c.case_num = ud.casenum
	join [sma_TRN_Cases] cas
		on TRY_CONVERT(UNIQUEIDENTIFIER, cas.source_id) = c.casesid
	join IndvOrgContacts_Indexed ioc
		on TRY_CONVERT(UNIQUEIDENTIFIER, ioc.source_id) = c.counselnamesid
	join IndvOrgContacts_Indexed iocp
		on TRY_CONVERT(UNIQUEIDENTIFIER, iocp.source_id) = c.representingnamesid
...
insert into [sma_TRN_LawFirms]
	...
	join IndvOrgContacts_Indexed iocd
		on TRY_CONVERT(UNIQUEIDENTIFIER, iocd.source_id) = c.representingnamesid
...
from (
	...
	from [BenAbbot_Needles].[dbo].counsel_Indexed C
	join [BenAbbot_Needles].[dbo].[names] L
		on C.counselnamesid = L.id
	join [dbo].[sma_MST_IndvContacts] I
		on I.source_id = CONVERT(VARCHAR(50), L.id)
	where L.person = 1
) A
