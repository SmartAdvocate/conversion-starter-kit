...
--		null,
--		0
--	from [BenAbbot_Needles].dbo.cases_Indexed as c
...
insert into sma_TRN_caseStaff
	...
	select distinct
		...
		0				as cssnLevelNo
	--SELECT *
	from [BenAbbot_Needles]..case_staff cs
	join [BenAbbot_Needles]..matter_staff ms
		on ms.id = cs.matterstaffid
	join [BenAbbot_Needles]..staff_Role sr
		on sr.id = ms.staffroleid
	join [sma_TRN_cases] CAS
		on CAS.source_id = CONVERT(VARCHAR(50), cs.casesid)
	join [sma_MST_Users] U
		on (U.source_id = CONVERT(VARCHAR(50), cs.staffid))
go
