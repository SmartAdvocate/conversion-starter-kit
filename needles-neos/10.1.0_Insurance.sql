...
alter table [sma_TRN_InsuranceCoverageAdjusters] disable trigger all
delete from [sma_TRN_InsuranceCoverageAdjusters]
--- DBCC CHECKIDENT ('[sma_TRN_InsuranceCoverageAdjusters]', RESEED, 0);
alter table [sma_TRN_InsuranceCoverageAdjusters] disable trigger all
...
IF NOT EXISTS (SELECT * FROM sys.columns WHERE Name = N'source_ref' AND Object_ID = Object_ID(N'sma_TRN_InsuranceCoverage'))
BEGIN
    ALTER TABLE [sma_TRN_InsuranceCoverage] 
	ADD [source_ref] varchar(50); 
END
...
FROM  [NeosBillEasterly]..user_insurance_data td
...
--SELECT *
FROM  [NeosBillEasterly].[dbo].[insurance_Indexed] INS
...
FROM [NeosBillEasterly] ..[insurance_Indexed] INS
JOIN [sma_TRN_cases] CAS on CAS.Neos_saga = convert(varchar(50),INS.casesid)
...
SELECT 'Unspecified'
UNION
SELECT DISTINCT [type] FROM  [NeosBillEasterly]..[insurance] INS 
JOIN  [NeosBillEasterly]..[insurance_type] it on it.id = ins.insurancetypeid
EXCEPT
...
(
	...
	[incnUnInsPolicyLimitAcc],[incnUnderPolicyLimitAcc],[incb100Per],[incnMVLeased],[incnPriority],[incbDelete],[incnauthtodefcoun],[incnauthtodefcounDt],[incbPrimary],[saga], source_ref
)
SELECT 
	...
	isnull('Insurance Information: ' + nullif(convert(varchar,ni.[InsurancePlanType]),'') + CHAR(13),'') +
	isnull('MedPay Coverage: ' + nullif(convert(varchar,ni.[FinalDemandAmount]),'') + CHAR(13),'') +
	isnull('PIP Coverage: ' + nullif(convert(varchar,ni.[FinalSettlementDetail]),'') + CHAR(13),'') +
	isnull('PIP Exhausted: ' + nullif(convert(varchar,ni.[SetUpClaimWithCOBC]),'') + CHAR(13),'') +
	isnull('PIP Waiver: ' + nullif(convert(varchar,ni.[PORFromClient]),'') + CHAR(13),'') +
	isnull('UM/UIM Coverage: ' + nullif(convert(varchar,ni.[NameOfTheGroup]),'') + CHAR(13),'') +
	isnull('Other Household Vehicles: ' + nullif(convert(varchar,ni.[CPLReceived]),'') + CHAR(13),'') +   
	 ''			    as [incsComments],
...
--    ( select plnnPlaintiffID from sma_TRN_Plaintiff where plnnCaseID=MAP.caseID and plnbIsPrimary=1 )  
	...
	0					as [incbPrimary],
	INS.id				as [saga],
	'Neos..[insurance_Indexed].id'    as source_ref
--SELECT max(len([claim]))
FROM [NeosBillEasterly] .[dbo].[insurance_Indexed] INS 
LEFT JOIN  [NeosBillEasterly].[dbo].[insurance_type] it on it.id = ins.insurancetypeid
...
LEFT JOIN  [NeosBillEasterly]..NeosUserInsurance ni on ni.insuranceid = ins.id
GO
...
FROM sma_TRN_InsuranceCoverage ic
JOIN  [NeosBillEasterly]..[insurance_Indexed] INS on convert(varchar(50),INS.ID) = IC.saga
...
SELECT incnInsCovgID, ioc2.UNQCID
FROM sma_TRN_InsuranceCoverage ic
JOIN [NeosBillEasterly] ..[NeosUserInsurance] INS on convert(varchar(50),INS.insuranceid) = IC.saga
