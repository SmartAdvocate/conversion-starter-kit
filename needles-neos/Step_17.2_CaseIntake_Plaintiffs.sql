...
From  [NeosBillEasterly]..case_intake
...
SELECT 4, '(P)-Plaintiff', cst.cstnCaseTypeID, (select srcnCodeId from sma_MST_SubRoleCode where srcsDscrptn = '(P)-Plaintiff' and srcnRoleID = 4)
FROM sma_MST_CaseType cst
JOIN sma_trn_cases cas on cas.casnOrgCaseTypeID = cst.cstnCaseTypeID
...
(
 ...
 [plnnLIPBatchNo],[plnnPlaintiffRole],[plnnPlaintiffGroup],[plnnPrimaryContact], source_id, source_db,source_ref,Needle_Saga
)
SELECT DISTINCT
	...
	(Select top 1 sbrnSubRoleId	from [sma_MST_SubRole] where sbrnCaseTypeID =  CAS.casnOrgCaseTypeID and sbrnRoleID = 4 and sbrsDscrptn = '(P)-Plaintiff' )		as [plnnRole],
	1						as [plnbIsPrimary],
	...
	''     as source_id,
	'Needle'    as source_db,
	' [NeedlesSamElderLaw]..case_intake'    as source_ref,
	 c.ROW_ID 					as [Needle_Saga]
--select *
FROM   [NeosBillEasterly]..case_intake       c
JOIN [sma_TRN_Cases] CAS on CAS.saga = CAST(ROW_ID AS VARCHAR) 
JOIN IndvOrgContacts_Indexed CIO on CIO.saga_ref = c.name_id
WHERE isnull(name_id,0)<>0
