...
(	SELECT DISTINCT [description] as [name]
	FROM [NeosBillEasterly].[dbo].[class] class
	JOIN [NeosBillEasterly].[dbo].[cases] C on C.classid = class.[id]
...
SELECT
    ...
    ''				as [cssnExpDays],
   c.date_opened	as [cssdFromDate],
    DATEDIFF(DAY, c.date_opened, GETDATE())		as [cssdToDt],
    case when C.close_date between '1900-01-01' and '2079-06-06' 
...
JOIN  [NeosBillEasterly].[dbo].[cases] C on convert(varchar(50),C.[id]) = CAS.Neos_saga
JOIN [NeosBillEasterly] .[dbo].[class] CL on C.classid = CL.id
GO
